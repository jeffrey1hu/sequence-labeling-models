...
# -*- coding: utf-8 -*-
"""
A window base nerual network model for NER
"""
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for idx, (term_vec, label) in enumerate(zip(sentence, labels)):
            start_num = max(window_size - idx, 0)
            end_num = max(window_size - (len(sentence) - (idx + 1)), 0)

            start_idx = max(idx - window_size, 0)
            end_idx = min(idx + window_size + 1, len(sentence))

            sentence_piece = reduce(lambda _x, _y: _x+_y,sentence[start_idx:end_idx])
            x = start_num * start + sentence_piece + end_num * end
            windowed_data.append((x, label))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        assert (_input_embeddings.get_shape()[1], _input_embeddings.get_shape()[2]) == (self.config.n_window_features, self.config.embed_size)
        embeddings = tf.reshape(_input_embeddings, shape=(-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        relu_h = tf.nn.relu(h)
        h_drop = tf.nn.dropout(relu_h, keep_prob=dropout_rate)
    ...
    def add_loss_op(self, pred):
        ...
        labels = tf.one_hot(self.labels_placeholder, depth=self.config.n_classes)
