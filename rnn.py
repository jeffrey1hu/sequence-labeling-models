...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(new_sentence) < max_length:
            bias = (max_length - len(new_sentence))
            new_sentence = new_sentence + bias * [zero_vector]
            new_labels.extend([zero_label] * bias)
            mask[len(sentence):] = [False] * bias
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(_input_embeddings, shape=(-1, self.max_length, Config.n_features * Config.embed_size))
        ### END YOUR CODE
        return embeddings
